...
function Res(socket, ack) {
  ...
  this.end = function() {
    if (this.ack) socket.write(VT + (this.ack).toString() + FS + CR);
  }
}
...
TcpServer.prototype.start = function(port, encoding, options) {
  ...
  this.server = net.createServer(function(socket) {
    ...
    if (options.onconnect) options.onconnect(socket);
    ...
    socket.on('close', function() {
      console.log('Socket close', socket.remoteAddress);
      if (options.onclose) options.onclose(socket);
    })
  });
